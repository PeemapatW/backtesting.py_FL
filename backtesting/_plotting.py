...
def plot_intnode(*, results: pd.Series,
         df: pd.DataFrame,
         indicators: List[_Indicator],
         filename='', plot_width=None,
         plot_equity=True, plot_return=False, plot_pl=True,
...
return this.labels[index] || "";
    ...
    def _plot_equity_section(is_return=False):
        ...
        equity = equity_data['Equity'].copy()
        dd_end = equity_data['DrawdownDuration'].idxmax()
        init_equity = equity.values[0]
        close = df['Close'].copy()
        init_close = close.values[0]
        bh_equity = [close.values[i]*init_equity/init_close for i in range(len(close))]
        print(len(bh_equity),len(equity))
        ...
        yaxis_label = 'Return' if is_return else 'Equity'
        source_key = 'eq_return' if is_return else 'equity'
        source_key_bh = 'eq_return_bh' if is_return else 'equity_bh'
        source.add(equity, source_key)
        source.add(bh_equity, source_key_bh)
        fig = new_indicator_figure(
            y_axis_label=yaxis_label,
            **({} if plot_drawdown else dict(height=110)))
        ...
        # Equity line
        r = fig.line('index', source_key, source=source, line_width=1.5, line_alpha=1)
        r_bh = fig.line('index', source_key_bh, source=source, line_width=1.5, line_alpha=1, color='salmon', line_dash='dashed')
        if relative_equity:
            tooltip_format = f'@{source_key}{{+0,0.[000]%}}'
            tooltip_format_bh = f'@{source_key_bh}{{+0,0.[000]%}}'
            tick_format = '0,0.[00]%'
            legend_format = '{:,.0f}%'
        else:
            tooltip_format = f'@{source_key}{{$ 0,0}}'
            tooltip_format_bh = f'@{source_key_bh}{{$ 0,0}}'
            tick_format = '$ 0.0 a'
            legend_format = '${:,.0f}'
        set_tooltips(fig, [(yaxis_label, tooltip_format),('Market',tooltip_format_bh)], renderers=[r])
        fig.yaxis.formatter = NumeralTickFormatter(format=tick_format)
    ...
    for df in dfs:
        try:
            ...
            if name1.split('-')[0] == name2.split('-')[0]:
                ...
                param_name1 = name1.split('-')[-1]
                param_name2 = name2.split('-')[-1]
